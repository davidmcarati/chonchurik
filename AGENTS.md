...
- Selecting parameters on profit is permitted only as a disclosed experiment, in `tools/evolve`, over declared free physiological parameters — never over wiring, never inside a live run, and never as the runtime default. It must publish its chronological split, its four baselines, its pre-declared kill criterion and the whole final population out of sample, and it must say what a champion is not.
